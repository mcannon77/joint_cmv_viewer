...
// *
// *  Copyright  2007,2008  Larry Moore
// *  jane.larry@gmail.com
// *  last change or bug fix: December 11, 2008
// ****************************************************************************/
//
...
// negative numbers for Y values in the southern hemisphere.  The calling application must check for
...
define([], function(){
  ...
  ***************************************************************************/
  var LLtoUTM = function(lat,lon,utmcoords,zone) {
  ...
  ***************************************************************************/
  ...
  var LLtoUSNG = function(lat, lon, precision) {
  ...
  ***************************************************************************/
  ...
  var UTMtoLL = function(UTMNorthing, UTMEasting, UTMZoneNumber, ret) {
  ...
  ***********************************************************************************/ 
  ...
  var USNGtoUTM = function(zone,let,sq1,sq2,east,north,ret) { 
  ...
  var USNGtoLL = function(usngStr_input,latlon) {
  ...
  var parseUSNG_str = function(usngStr_input, parts)
  {
     var j = 0;
     var k;
     var usngStr = [];
  ...
  // checks a string to see if it is valid lat/lng in D-M.mmm format
  //    if so, returns a new GLatLng
  //    if not, returns 0
  var isLatLngDM = function(inputStr) {
     var j = 0;
     var k;
     var dmStr = [];
     var strregexp
     ...
     if (dmStr.match(strregexp)) {
        parts = strregexp.exec(dmStr.match(strregexp));
        var lat = parts[1]*1+(parts[2]/60);
        var lng = parts[7]*1+(parts[8]/60);
        if (parts[4] == 'S') {
           lat *= -1;
        }
        if (parts[10] == 'W') {
  ...
  //      alert(parts[1]+" "+parts[2]+" "+parts[4]+" "+parts[7]+" "+parts[8]+" "+parts[10]+" "+lat+", "+lng);

        return(new GLatLng(lat,lng))
     }

     // else if matches longitude/latitude
     strregexp = new RegExp("^([0-9]{1,3})-([0-9]{1,2}(\.[0-9]*)?)([W|E]+)( *)\,( *)([0-9]{1,3})-([0-9]{1,2}(\.[0-9]*)?)([N|S]+) *")
     if (dmStr.match(strregexp)) {
  //      alert("match(2): "+dmStr);
        parts = strregexp.exec(dmStr.match(strregexp));
        var lng = parts[1]*1+(parts[2]/60);
        var lat = parts[7]*1+(parts[8]/60);
        if (parts[4] == 'W') {
           lng *= -1;
        }
        if (parts[10] == 'S') {
  ...
  var LLtoMGRS = function(lat, lon, precision)
  {
     var mgrs_str="";
  ...
  // unfinished...start of one function that will parse input for all cases of usng, mgrs, and lat/lng
  var isCartoCoordinate = function(inputStr,latlon) {
     // latlon is a 2-element array declared by calling routine
  ...
  var LLtoUTMString = function(lat, lon) {
      //Zone 10 S 559741 mE. 4282182 mN.
      var _array = [0, 0, 0];
      var _formattedString = "";

      LLtoUTM(lat, lon, _array, null);
      var _hemi = lat < 0 ? "S" : "N";
      var _easting = Math.round(_array[0], 0);
      var _northing = Math.round(_array[1], 0);
      var _zone = _array[2];
      _formattedString = "Zone " + _zone + " " + _hemi + " " + _easting + " mE. " + _northing + " mN.";
  ...
  var output = {
    ...
    isUSNG: isUSNG,
    isLatLngDM: isLatLngDM,
    LLtoMGRS: LLtoMGRS,
    GUsngtoLL: GUsngtoLL,
    isCartoCoordinate: isCartoCoordinate,
    LLtoUTMString: LLtoUTMString
  };
