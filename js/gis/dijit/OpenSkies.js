define([
    ...
    'dojo/text!./OpenSkies/templates/OpenSkies.html',
    './OpenSkies/DroppedCSV',
    'dojo/i18n!./OpenSkies/nls/resource',
    'dojox/form/FileInput',
    'xstyle/css!./OpenSkies/css/OpenSkies.css'
], function (
        put,
        request, Graphic, InfoTemplate, urlUtils, Color,
        Point, Multipoint, webMercatorUtils, scaleUtils,
        ArcGISDynamicMapServiceLayer, ArcGISImageServiceLayer, FeatureLayer, GraphicsLayer,
        PictureMarkerSymbol, SimpleLineSymbol, SimpleFillSymbol,
        SimpleRenderer, SimpleMarkerSymbol, TextSymbol, LabelLayer, Polyline, SpatialReference, geodesicUtils,
        array, declare, lang,
        on,
        CsvStore, base64,
    ...
    /*    handleZip: function (file) {
        ...
        processCSVData: function (data, filename) {
            ...
            csvStore.fetch({
                ...
                var flightPath = new GraphicsLayer({id:"flightpath"});
			//	var flightNode = new GraphicsLayer({id:"node"});				
				var white = new Color([255, 255, 255, 1.0]); // hex is #ff4500
				var sls = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([255, 0, 0]), 1.5);
                var marker = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, 15, sls, white);
                var renderer = new SimpleRenderer(marker);
                var labelField = "__OBJECTID";

				var pLabel = new TextSymbol();
					pLabel.font.setSize("6pt");
					pLabel.font.setFamily("arial");
					//pLabel.align.setAlign(TextSymbol.ALIGN_START);
					//pLabel.horizontalAlignment = "justify";
					//pLabel.font.setWeight("WEIGHT_BOLDER");
					pLabel.setOffset(-10,-13);		
				
				var csvArray = [];
				var polyLine = new Polyline(new SpatialReference({wkid: 4326}));		
				var labelrenderer = new SimpleRenderer(pLabel);
				...
				var template = new InfoTemplate("Attributes", "${*}");		
                    ...
                    array.forEach(items, lang.hitch(this, function (item) {
						...
						//console.log(geometry);
                        			var geometry2 = new Point(longitude, latitude);
						//console.log(geometry2);
						//polyLine.clearCache();
						csvArray.push([geometry2.x,geometry2.y]);
						
//						var newNode = new Graphic(geometry2);
//						flightNode.setRenderer(renderer);
//						flightNode.setInfoTemplate(template);
//					    flightNode.add(newNode);
						
						if ((csvArray.length % 2) == 0 ) {
						if(csvArray[0][0] < csvArray[1][0]) {
						console.log(csvArray);
										
						var polyLine = new Polyline(new SpatialReference({wkid: 4326}));
											
						polyLine.addPath(csvArray);

						var clientDensifiedGeometry = new geodesicUtils.geodesicDensify(polyLine, 5000);

						var newGraphic = new Graphic(clientDensifiedGeometry, sls, null, null);
						flightPath.add(newGraphic);
						this.map.addLayer(flightPath);
						//csvArray.length = 0;
						csvArray.shift();
						}
						else {										
                  ...
                  //  featureLayer.__popupInfo = popupInfo;
                    ...
                    labels.addFeatureLayer(featureLayer, labelrenderer, "{" + labelField + "}");
					//console.log(labels);
